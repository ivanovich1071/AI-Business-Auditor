...
export default function DashboardPage() {
  ...
  const [openResult, setOpenResult] = useState<AnalysisResult | null>(null);
  ...
  async function handleDelete(id: string) {
    ...
    if (openResult?.id === id) setOpenResult(null);
        ...
        {industryStats.length > 0 && (
          <div className="mt-6 flex flex-wrap gap-2">
            {industryStats.map(([industry, count]) => (
              <span key={industry} className="rounded-full bg-accent-warm/5 px-3 py-1 text-sm text-accent-warm/70">
                {industry} — {count}
              </span>
            ))}
          </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {!loading && filtered.length === 0 && (
            <p className="text-accent-warm/50">
              Пока нет сохранённых анализов. <Link href="/" className="underline">Проанализировать сайт</Link>
            </p>
          )}
          {filtered.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between rounded-2xl border border-accent-warm/10 bg-white/60 p-4 shadow-lg shadow-accent-warm/5 transition-all duration-300 hover:-translate-y-0.5"
            >
              <div>
                <p className="font-semibold">{item.companyName}</p>
                <p className="text-sm text-accent-warm/60">
                  {item.url} · {item.industry ?? "отрасль не определена"} · {item.agentsCount} агентов
                </p>
